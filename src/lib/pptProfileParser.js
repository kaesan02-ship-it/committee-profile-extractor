...
const EDUCATION_TAIL_SECTION_PATTERN = /\s*(?:[➢>▶▷]\s*)?(?:경력사항\s*및\s*수행실적|경력\s*사항\s*및\s*수행\s*실적|주요경력|주요\s*경력|주요이력|주요\s*이력|경력사항|경력|수행실적|수행\s*실적|주요실적|주요\s*실적|전문분야|전문\s*분야)[\s\S]*$/i;
...
const PREVIOUS_PREFIX_PATTERN = /^(?:前|\(전\)|전(?=\s+|[-)\]:：.]))\s*[-)\]:：.]*/;
...
const prepareEducationSource = (text = '') => {
  return compactEducationSpacing(text)
    .replace(EDUCATION_TAIL_SECTION_PATTERN, '')
